...
筛选仓库后，画面只保留所选城区并自动居中适配视口，其他城区不会作为灰色背景保留。切回全部仓库即可恢复整座城市。

建筑具有窗格、入口雨棚、层间线与屋顶细节，并按尺寸与类型呈现退台高楼、低层楼房和坡屋顶建筑。2.5D、3D 与导出的横幅共用建筑规则；密集城区会减少细节数量，同时保留建筑轮廓和窗户。
...
| 城区内的街区 | 一级目录                                                                       |
| 建筑         | 一个有效文本文件                                                               |
| 建筑高度     | 行数或文件字节数，采用幂函数缩放，拉开小文件与大文件的高度差异                 |
| 建筑占地     | 文件大小的相对权重                                                             |
| 建筑颜色     | 文件语言                                                                       |
| 绿色屋顶     | 测试文件                                                                       |
| 建筑房型     | 随高度分为小型附属建筑、坡顶住宅、联排住宅、展馆、中层楼、退台楼、高塔和摩天楼 |
| 地标塔尖     | 仓库 Star 数量；每个有 Star 的非空仓库一处，采用有上限的对数缩放               |
| 窗户亮度     | 文件最近提交时间相对于所扫描仓库 commit 的活跃程度                             |
| 灰色城区     | 匿名展示的私有仓库，或已归档仓库；仍然对应真实文件                             |

多个仓库一起展示时，仓库名称位于城市两侧，通过编号和引线连接所属城区。公开文件的悬停提示包含仓库名；私有建筑保持无详情、无文件名、无跳转。

所有仓库使用同一高度尺度，筛选仓库不会改变楼高。以行数为例，50 行、1,000 行和 2,000 行的建筑高度约为 10、72 和 122 个绘图单位；极大文件的高度设有上限，避免遮住整座城市。详情面板始终显示原始行数和文件大小。
